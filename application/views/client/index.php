<div class="pageTitle">顧客資料維護</div>

<div class="pageContent">
    <div class="row">
        <div class="col-md-4">
            <label>顧客編號：</label>
            <input type="text" id="clientNo" maxlength="20" value="<?php if (isset($searchData['clientNo'])) echo $searchData['clientNo']; ?>">
        </div>
        <div class="col-md-4">
            <label>公司名稱：</label>
        ...
        <div class="col-md-4">
            <label>統一編號：</label>
            <input type="text" id="companyId" maxlength="10" value="<?php if (isset($searchData['companyId'])) echo $searchData['companyId']; ?>">
        </div>
    </div>
    <div class="btnBox">
...
<div class="pageContent">
    <div class="addButBox">
        <button class="button" onclick="gotoAdd();">新增顧客資料</button>
    </div>
    <!-- 列表  -->
    <div class="contentsList" id="dataList">
        <?php
        $no = ($searchData['page'] - 1) * $searchData['pageCount'];
        $pageTotalCount = count($data['client']);
        if ($pageTotalCount != 0) {
            //表頭
            ...
            echo '<th>統一編號</th>';
            echo '<th>顧客代表</th>';
            echo '<th class="width_80px">聯絡資訊</th>';
            ...
            foreach ($data['client'] as $row) {
                ...
                //聯絡資訊
                $conBtn = "<button class='button gray' onclick='viewCon(" . $rowStr . ");'>檢視</button>";
                echo '<td class="textCenter">' . $conBtn . '</td>';
                ...
                $delBtn = "<button class='button removeRed' onclick='delClient(" . $rowStr . "," . $pageTotalCount . ");'>刪除</button>";
                echo '<td class="textCenter">' . $editBtn . $delBtn . '</td>';
                echo '</tr>';
            }
            echo '</table>';
        } else {
            echo '<div class="textRed textCenter">查無顧客資料</div>';
        }
        ?>
    </div>
